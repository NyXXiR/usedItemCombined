	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == joinBtn) { // 가입버튼 눌렀을 때
			if (newId.trim().equals("") || newPwd.trim().equals("") || newPwdRe.trim().equals("")
				...
				if (!logInPage.userDao.getIdByCheck(newId)) { // 중복일 경우
					...
					if (newPhone != 0) {
						User newUser = new User(newId, newPwd, newName, newAddress, newNick, newPhone);
						// ArrayList에도 넣기.
						logInPage.userDao.insertUserDB(newUser);
			...
			if (newId.trim().equals("")) {
				...
				if (logInPage.userDao.getIdByCheck(newId)) { // 중복아니다.
